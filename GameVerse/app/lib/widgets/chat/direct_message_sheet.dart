...
Future<void> showDirectMessageSheet(
  ...
  required String avatarUrl,
      ...
      builder: (_) => _DirectMessageSheet(userId: userId, username: username, avatarUrl: avatarUrl),
    );

class _DirectMessageSheet extends StatefulWidget {
  const _DirectMessageSheet({required this.userId, required this.username, required this.avatarUrl});
...
class _DirectMessageSheetState extends State<_DirectMessageSheet> {
  ...
  late Future<List<DirectMessage>> _messages = _service.getConversation(widget.userId);
  bool _sending = false;
  ...
  Widget build(BuildContext context) => DraggableScrollableSheet(
        ...
        maxChildSize: .94,
        expand: false,
        builder: (context, sheetController) => Column(children: [
          Container(
            width: 42,
            height: 4,
          ...
          Padding(
            ...
            child: Row(children: [
              CircleAvatar(
                ...
                backgroundImage: widget.avatarUrl.isEmpty ? null : NetworkImage(widget.avatarUrl),
                child: widget.avatarUrl.isEmpty ? Text(widget.username.isEmpty ? '?' : widget.username[0].toUpperCase(), style: const TextStyle(color: Colors.white)) : null,
              ),
              const SizedBox(width: 12),
              Expanded(child: Text(widget.username, style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w700))),
          ...
          Expanded(
            child: FutureBuilder<List<DirectMessage>>(
              ...
              builder: (context, snapshot) {
                ...
                if (snapshot.hasError) return Center(child: Text('No se pudieron cargar los mensajes.', style: const TextStyle(color: Colors.white70)));
                final messages = snapshot.data ?? [];
                if (messages.isEmpty) return const Center(child: Text('Aún no hay mensajes. Saluda primero.', style: TextStyle(color: Colors.white54)));
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Align(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(color: mine ? const Color(0xFF6D35F5) : const Color(0xFF292437), borderRadius: BorderRadius.circular(14)),
            ...
            child: Padding(
              ...
              child: Row(children: [
                ...
                IconButton(
                  ...
                  color: Colors.white,
                  style: IconButton.styleFrom(backgroundColor: const Color(0xFF6D35F5)),
                ),
              ]),
            ),
          ),
